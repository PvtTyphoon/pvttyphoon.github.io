...
function displayLastfmInfo(track) {
    ...
    if (isNowPlaying) {
        lastfmTextElement.innerHTML = `&nbsp;Currently listening to <span style="color: #ff6767; text-shadow: 0 0 10px #ff6767;">${songName}</span> by <span style="color: #ff6767; text-shadow: 0 0 10px #ff6767;">${artistName}</span>`;
    } else {
        const listenedTime = new Date(track.date.uts * 1000);
        const currentTime = new Date();
        const timeDifference = Math.floor((currentTime - listenedTime) / (1000 * 60));
        ...
        if (timeDifference < 10) {
            ...
            lastfmTextElement.innerHTML = `&nbsp;Was listening to <span style="color: #ff6767; text-shadow: 0 0 10px #ff6767;">${songName}</span> by <span style="color: #ff6767; text-shadow: 0 0 10px #ff6767;">${artistName}</span> ${timeDifference} minutes ago`;
        } else {
...
document.getElementById("button-close").addEventListener("click", playButtonClickSound);
document.getElementById("button-minimize").addEventListener("click", playButtonClickSound);
document.getElementById("button-maximize").addEventListener("click", playButtonClickSound);
